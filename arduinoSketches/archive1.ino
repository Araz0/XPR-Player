...
// #define int buttonPin4 = A4;  // the number of the pushbutton pin
// #define int buttonPin5 = A5;  // the number of the pushbutton pin
...
void setup() {
  ...
  pinMode(A4, INPUT);
  pinMode(A5, INPUT);
  pinMode(2, OUTPUT); // initialization of output 2 as "Output"
  pinMode(3, OUTPUT); // initialization of output 3 as "Output"
...
// read the state of the pushbutton value:
  ...
  if (buttonState1 < 10) {
    ...
    digitalWrite(ledPin, LOW);
    Serial.println("1");
    sendDataOverHttp("/api?selectedIndex=0");
    digitalWrite(2, HIGH); // LED on the output 2
    delay(500); // pause in program execution by half a second
    digitalWrite(2, LOW); // LED off on output 2
  } else if (buttonState2 < 10) {
    // button clicked:
    digitalWrite(ledPin, LOW);
    Serial.println("2");
    sendDataOverHttp("/api?selectedIndex=1");
    digitalWrite(3, HIGH); // LED on the output 3
    delay(500); // pause in program execution by half a second
    digitalWrite(3, LOW); // LED off on output 3
  } else if (buttonState3 < 10) {
    // button clicked:
    digitalWrite(ledPin, LOW);
    Serial.println("3");
    sendDataOverHttp("/api?customParam=1");
    delay(500);
  } else if (buttonState4 < 10) {
    // button clicked:
    digitalWrite(ledPin, LOW);
    Serial.println("4");
    sendDataOverHttp("/api?customParam=2");
    delay(500);
  } else if (buttonState5 < 10) {
    // button clicked:
    digitalWrite(ledPin, LOW);
    Serial.println("4");
    sendDataOverHttp("/api?customParam=3");
    delay(500);
  }  else {
    // turn LED On:
    digitalWrite(ledPin, HIGH);
